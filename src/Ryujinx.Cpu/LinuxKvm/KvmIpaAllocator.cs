...
namespace Ryujinx.Cpu.LinuxKvm
{
    class KvmIpaAllocator
    {
        ...
        public ulong Allocate(ulong size, ulong alignment = AllocationGranule)
        {
            ...
            {
                throw new InvalidOperationException($"Not enough free IPA memory to allocate 0x{size:X} bytes with alignment 0x{alignment:X}.");
            }
